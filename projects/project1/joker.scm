...
(define (make-ordered-deck)
  ...
  (se (make-suit 'H) (make-suit 'S) (make-suit 'D) (make-suit 'C) '(new_joker new_joker) )) ;added 2 jokers into the deck.
...
(define (make-deck)
  ...
  (shuffle (make-ordered-deck) 54) ) ;updated the number of card(add 2 jokers)

;updated best-total
(define (best-total hand)
(define (sort hand) ;it will sort the hand as follows jokers --> aces --> other type of cards.
    (cond ((empty? hand) '())
          ((equal? (first hand) 'new_joker) (se (first hand) (sort (bf hand))))
      ((and (member? (bl (first hand)) '(A a)) (not (member? 'new_joker hand)) (not (empty? (bf hand)))) (se (first hand) (sort (bf hand))))
      ((and (member? (bl (first hand)) '(A a)) (not (empty? (bf hand)))) (sort (se (bf hand) (first hand))))
      (else (se (sort (bf hand)) (first hand)))))
    (define (best-total-helper hand total)
     (cond ((equal? hand '()) total)
           ((and (equal? (last hand) 'new_joker) (< total 21) (> total 9) (empty? (bl hand))) (best-total-helper (bl hand) (+ total (- 21 total))))
           ((and (equal? (last hand) 'new_joker) (< total 21) (> total 9)) (best-total-helper (bl hand) (+ total 1))) ;there is one more joker in the hand so in order to achieve 21, first add 1 then at the following call, add 10.
           ((and (equal? (last hand) 'new_joker) (< total 10)) (best-total-helper (bl hand) (+ total 11)))
           ((and (equal? (last hand) 'new_joker) (>= total 21)) (best-total-helper (bl hand) (+ total 1)))
           ((and (member? (bl (last hand)) '(A a))(< total 11))  (best-total-helper (bl hand) (+ total 11)))
           ((and (member? (bl (last hand)) '(A a)) (>= total 11))  (best-total-helper (bl hand) (+ total 1)))
           ((member? (bl (last hand)) '(k q j K Q J)) (best-total-helper (bl hand) (+ total 10)))
           (else (best-total-helper (bl hand) (+ total (butlast (last hand)))))))
(best-total-helper (sort hand) 0))
...
(check-expect equal? (best-total '(as as ah)) 13)
(check-expect equal? (best-total '(new_joker 4s)) 15)
(check-expect equal? (best-total '(8s new_joker 4s)) 21)
(check-expect equal? (best-total '(8s new_joker ad)) 21)
(check-expect equal? (best-total '(ad new_joker)) 21)
(check-expect equal? (best-total '(ad new_joker 3s)) 21)
(check-expect equal? (best-total '(new_joker)) 11)
(check-expect equal? (best-total '(new_joker ad)) 21)
(check-expect equal? (best-total '(ad ad new_joker)) 21)
(check-expect equal? (best-total '(new_joker new_joker)) 21)
(check-expect equal? (best-total '(ad new_joker new_joker)) 21)
(check-expect equal? (best-total '(ad 9s new_joker)) 21)
(check-expect equal? (best-total '(ad 8s new_joker new_joker)) 21)
(check-expect equal? (best-total '(8h 10s 3c new_joker)) 22) ;bust(go over 21)
...
;4
(define (dealer-sensitive customer-hand up-card) ;added jokers into picture card
  (cond ((>= (best-total customer-hand) 17) #f)
        ((and (or (equal? (first up-card) 'a) (and (number? (first up-card)) (> (butlast up-card) 6))
             (member? (first up-card) '(k q j K Q J)) (equal? up-card 'new_joker))
...
(check-expect equal? (dealer-sensitive '(3s new_joker) 'new_joker) #t)
